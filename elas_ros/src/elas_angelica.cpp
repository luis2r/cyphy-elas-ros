...
public:
  Elas_Proc(const std::string& transport)
  {
    ...
    local_nh.param("queue_size", queue_size_, 100);
               ...
               const sensor_msgs::CameraInfoConstPtr& r_info_msg)
  {
    ...
    float* l_prob_data = new float[width*height*sizeof(float)];
    float* r_prob_data = new float[width*height*sizeof(float)];


    // Process
    elas_->process(l_image_data, r_image_data, l_disp_data, r_disp_data, l_prob_data, r_prob_data, dims);
    ...
    out_depth_msg.image = cv::Mat(height, width, CV_8UC1);
    ...
    for (int32_t i=0; i<width*height; i++)
    {
      ...
      // out_msg.image.data[i] = (uint8_t)std::max(255.0*l_prob_data[i],0.0);
      ...
      out_depth_msg.image.data[i] = (uint8_t)std::max(255.0*l_prob_data[i],0.0);
    }
